...
Instagram **스토리 광고 → LP 클릭 플로우** UT용 HTML 프로토타입.
...
`index.html`(A/B/C 선택) → `feed.html?h=N`(**인스타 스토리** 광고, 3장 자동/탭 넘김)
→ 링크 스티커 「詳しくはこちら →」 탭 또는 끝까지 시청 → 해당 가설 기획전 LP
...
| `feed.html?h=1\|2\|3` | Instagram **스토리** 광고 플레이어(3장) |
...
광고는 **가설당 3장 스토리 세그먼트**(`STORY_MS`=5초 자동 넘김, 좌/우 탭으로 이전·다음).
정사각/세로 소재는 화면에 contain + 동일 이미지 블러 배경으로 풀스크린 표시(실제 IG 스토리 동작).
